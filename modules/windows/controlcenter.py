...
class ControlSwitchPill(Gtk.Button):
    ...
    def __init__(self):
        super().__init__()
        self.set_css_classes(["m-1", "rounded"])

        builder = Gtk.Builder.new_from_file(ensure_ui_file("controlcenter/switchpill"))
        self._pill: Gtk.Box = builder.get_object("pill")  # type: ignore
        self._icon: Gtk.Image = builder.get_object("icon")  # type: ignore
        self._title: Gtk.Inscription = builder.get_object("title")  # type: ignore
        self._subtitle: Gtk.Inscription = builder.get_object("subtitle")  # type: ignore
        self._action: Gtk.Button = builder.get_object("action")  # type: ignore

        self.set_child(self._pill)
    ...
    def set_on_click(self, handler: Callable[..., Any], *args: Any):
        return self.connect("clicked", handler, *args)


class ControlSwitchCmd(ControlSwitchPill):
    ...
    def __init__(self):
        ...
        self.set_on_click(self.__on_clicked)

    @GProperty(type=str)
    def title(self) -> str:
        return self._title.get_text() or ""
    ...
    def action_icon(self, icon: str):
        ...
        if icon:
            ...
            self._action.set_icon_name(icon)
        else:
    ...
    def action_cmd(self, cmd: str):
        ...
        if cmd:
            self._action.connect("clicked", lambda *_: run_cmd_async(cmd))
    ...
    def __on_status_changed(self):
        if self._enabled:
            self.set_subtitle("enabled")
            self.set_style_accent(True)
        else:
            self.set_subtitle("disabled")
            self.set_style_accent(False)
...
class ColorSchemeSwitcher(ControlSwitchPill):
    __gtype_name__ = "ColorSchemeSwitcher"

    def __init__(self):
        super().__init__()

        self.set_icon("dark-mode-symbolic")
        self.set_title("Color Scheme")
        ...
        try:
            ...
            self.set_on_click(lambda *_: self.__switch_color_scheme(1))
            set_on_click(self, right=lambda *_: self.__switch_color_scheme(-1))
    ...
    def __on_color_scheme_changed(self, *_):
        ...
        self.set_subtitle(self.__color_scheme)
...
class IgnisRecorder(ControlSwitchPill):
    ...
    def __init__(self):
        ...
        self.set_subtitle("screen recorder")
        ...
        self.set_on_click(self.__on_clicked)
        set_on_click(self, right=self.__on_right_clicked)
        self.__on_status_changed()
    ...
    def __on_status_changed(self, *_):
        if self.__service.active:
            ...
            if self.__service.is_paused:
                ...
                self.set_icon("media-playback-stop-symbolic")
        else:
            self.set_style_accent(False)
            self.set_icon("screencast-recorded-symbolic")
...
class DndSwitch(ControlSwitchPill):
    ...
    def __on_option_changed(self, *_):
        if self.__group:
            if self.__group.dnd:
                self.set_subtitle("disable popups")
                self.set_style_accent(True)
                self.set_icon("notifications-disabled-symbolic")
            else:
                self.set_subtitle("default")
                self.set_style_accent(False)
                self.set_icon("notifications-symbolic")
...
class CaffeineSwitch(ControlSwitchPill):
    ...
    def __init__(self):
        ...
        self.set_title("Caffeine")
        self.set_subtitle("disabled")
        self.set_icon("my-caffeine-off-symbolic")
        self.set_tooltip_text("Click to toggle")
    ...
    def __on_changed(self, *_):
        ...
        self.set_subtitle("enabled" if enabled else "disabled")
        if enabled:
            self.set_icon("my-caffeine-on-symbolic")
            self.set_style_accent(True)
        else:
            self.set_icon("my-caffeine-off-symbolic")
            self.set_style_accent(False)
...
class PowerProfilesSwitch(ControlSwitchPill):
    __gtype_name__ = "PowerProfilesSwitch"

    def __init__(self):
        super().__init__()

        self.set_title("Power Profile")
        self.set_subtitle("Unavailable")
        self.set_icon("dialog-question-symbolic")
    ...
    def __on_changed(self, *_):
        self.set_subtitle(self.__service.active_profile)
        self.set_icon(self.__service.icon_name)
...
class EthernetStatus(ControlSwitchPill):
    ...
    def __on_status_changed(self, *_):
        self.set_icon(self.__ethernet.icon_name)
        self.set_tooltip_text()

        if not self.__ethernet.is_connected:
            self.set_subtitle("disconnected")
            self.set_style_accent(False)
            return
        ...
        match len(devices):
            case 0:
                self.set_subtitle("no device")
            case 1:
                device_name = devices[0].name
                self.set_subtitle(device_name)
                self.set_tooltip_text(device_name)
            case _:
                self.set_subtitle(f"{len(devices)} devices")


class WifiStatus(ControlSwitchPill):
    ...
    def __on_status_changed(self, *_):
        self.set_icon(self.__wifi.icon_name)
        self.set_tooltip_text()
...
class BluetoothStatus(ControlSwitchPill):
    ...
    def __on_status_changed(self, *_):
        if not self.__service.powered:
            self.set_subtitle("disabled")
            self.set_icon("bluetooth-disabled-symbolic")
            self.set_style_accent(False)
            return
        ...
        match len(devices):
            case 0:
                self.set_subtitle("disconnected")
                self.set_icon("bluetooth-disconnected-symbolic")
            case 1:
                alias = devices[0].alias
                self.set_subtitle(alias)
                self.set_icon(devices[0].icon_name)
                self.set_tooltip_text(alias)
            case _:
                self.set_subtitle(f"{len(devices)} devices")
